...
// Description: Layout 3 trên một dưới>:P . Trong 3 layout ở trên gồm một layout trái to và hai Layout phải nhỏ chia đôi(2 Layout bên phải một trên một dưới: Layout trên gồm image nền và title, image icon, Layout dưới gồm title, image icon, text content). Layout ở dưới to text chia làm 2 cột
...
-(void)initalizeViews:(NSArray*)viewCollectionDictonary{
    ...
    for (int i=0; i<[viewCollectionDictonary count]; i++) {
        ...
        if ([articalmode._Zone isEqualToString:@"Zone1"]) {
            view1 =[[[NewListArticleItemView2 alloc] initWithMessageModel: (ArticleModel*)[viewCollectionDictonary objectAtIndex:i]andViewoder:1] retain];
            ...
            view3 =[[[NewListArticleItemView2 alloc] initWithMessageModel: (ArticleModel*)[viewCollectionDictonary objectAtIndex:i]andViewoder:3] retain];
            
        }
    }
//===== test layout
    
//    view1 =[[[NewListArticleItemView3 alloc] initWithMessageModel: (ArticleModel*)[viewCollectionDictonary objectAtIndex:0]andViewoder:1] retain];
//    
...
-(void)rotate:(UIInterfaceOrientation)orientation animation:(BOOL)animation {
	...
	if (orientation == UIInterfaceOrientationPortrait || orientation == UIInterfaceOrientationPortraitUpsideDown) {
		if (view1 != nil) {
            ...
            [view1 setFrame:CGRectMake(-1, spaceHeder-1,773,371+spaceHeder+2)];
            [view2 setFrame:CGRectMake(-1,371+spaceHeder,384+1,527)];
            [view3 setFrame:CGRectMake(384, 371+ spaceHeder,384,527)];
		...
		if (view1 != nil) {		
            ...
            [view1 setFrame:CGRectMake(-1,spaceHeder-1,343+2,643)];
            [view2 setFrame:CGRectMake(343,spaceHeder-1,343,643)];
            [view3 setFrame:CGRectMake(343*2,spaceHeder-1,344,643)];
            [view1 reAdjustLayout:orientation];
            [view2 reAdjustLayout:orientation];
            [view3 reAdjustLayout:orientation];

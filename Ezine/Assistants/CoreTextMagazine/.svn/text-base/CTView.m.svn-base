...
- (void)buildFrames
{
    ...
    int columnIndex = 0;
    while (textPos < [attString length]) { //4
        CGPoint colOffset = CGPointMake( (columnIndex+1)*frameXOffset + columnIndex*(textFrame.size.width/2), 0 );
        CGRect colRect = CGRectMake(0, 0 , textFrame.size.width/2-20, textFrame.size.height-20);

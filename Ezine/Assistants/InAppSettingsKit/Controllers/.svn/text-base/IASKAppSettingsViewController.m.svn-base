...
//		[buttonItem release];
        UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithTitle:@"Hoàn thành" style:UIBarButtonItemStylePlain target:self action:@selector(dismiss:)];
        [anotherButton setTintColor:[UIColor colorWithRed:122.0f/255.0f green:160.0f/255.0f blue:234.0f/255.0f alpha:1.0]];
...
-(void)segmentValue:(id)sender{
    NSLog(@"segmentValue Click");
    UISegmentedControl *segment=(UISegmentedControl*)sender;
    if (segment.selectedSegmentIndex==0) {
        [[NSUserDefaults standardUserDefaults] setObject:@"magazine" forKey:@"typeListSite"];
        [segment setImage:[UIImage imageNamed:@"btn_SwitchMagazineChoose.png"] forSegmentAtIndex:0];
        [segment setImage:[UIImage imageNamed:@"btn_SwitchListNotChoose.png"] forSegmentAtIndex:1];
    }else{
        
        [[NSUserDefaults standardUserDefaults] setObject:@"list" forKey:@"typeListSite"];
        [segment setImage:[UIImage imageNamed:@"btn_SwitchListChoose.png"] forSegmentAtIndex:1];
        [segment setImage:[UIImage imageNamed:@"btn_SwitchMagazineNotChoose.png"] forSegmentAtIndex:0];
...
- (UITableViewCell*)newCellForIdentifier:(IASKSpecifier*)specifier {
    
    NSString *identifier=specifier.type;
	UITableViewCell *cell = nil;
	if ([identifier isEqualToString:kIASKPSToggleSwitchSpecifier]&&![specifier.title isEqualToString:@"Kiểu hiển thị"] ) {
	...
	}else if ([identifier isEqualToString:kIASKPSToggleSwitchSpecifier]&&[specifier.title isEqualToString:@"Kiểu hiển thị"] ){
        NSLog(@"kieu hien thi");
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kIASKPSToggleSwitchSpecifier];
		cell.accessoryView = [[[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"Tạp chí",@"Danh sách", nil]] autorelease];
		[((UISegmentedControl*)cell.accessoryView) addTarget:self action:@selector(segmentValue:) forControlEvents:UIControlEventValueChanged];
		cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(nil == cell) {
		cell = [[self newCellForIdentifier:specifier] autorelease];
	}
	NSLog(@"cell type=== %@   %@",specifier.type,specifier.title);
   	if ([specifier.type isEqualToString:kIASKPSToggleSwitchSpecifier]&&![specifier.title isEqualToString:@"Kiểu hiển thị"]) {
	...
	} //custom for Kiểu hiển thị
    else if ([specifier.type isEqualToString:kIASKPSToggleSwitchSpecifier]&&[specifier.title isEqualToString:@"Kiểu hiển thị"]){
        cell.textLabel.text = specifier.title;
		...
		UISegmentedControl *toggle = (UISegmentedControl*)cell.accessoryView;
        toggle.segmentedControlStyle=UISegmentedControlStyleBar;
        [toggle setTintColor:[UIColor grayColor]];
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"typeListSite"] isEqualToString:@"list"]){
            [toggle setSelectedSegmentIndex:1];
            [toggle setImage:[UIImage imageNamed:@"btn_SwitchListChoose.png"] forSegmentAtIndex:1];
            [toggle setImage:[UIImage imageNamed:@"btn_SwitchMagazineNotChoose.png"] forSegmentAtIndex:0];
            
        }else{
            [toggle setSelectedSegmentIndex:0];
            [toggle setImage:[UIImage imageNamed:@"btn_SwitchMagazineChoose.png"] forSegmentAtIndex:0];
            [toggle setImage:[UIImage imageNamed:@"btn_SwitchListNotChoose.png"] forSegmentAtIndex:1];
        }

        [toggle setFrame:CGRectMake(0, 0, 190, 27)];
    }
    //=================== end kieu hien thi================
	...
	} else if ([specifier.type isEqualToString:kIASKButtonSpecifier]&& ([specifier.key isEqualToString:@"btnFacebook"]||[specifier.key isEqualToString:@"btnEzine"]|| [specifier.key isEqualToString:@"customViewCell"])) {
...
//             [cell.detailTextLabel setHidden:NO];
            ...
            label1.font=[UIFont fontWithName:@"Arial-Bold" size:15];
             UILabel *label2=[[UILabel alloc] initWithFrame:CGRectMake(50, 23, 300, 20)];
             label2.text =@"Sản phẩm được phát triển bởi";
             label2.font=[UIFont fontWithName:@"Arial-Bold" size:15];
             ...
             UIImageView *moorelogo=[[UIImageView alloc] initWithFrame:CGRectMake(label2.frame.origin.x+label2.frame.size.width+10,label2.frame.origin.y+4,48,14)];
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([noSelectionTypes containsObject:specifier.type]) {
		return nil;
         //NSLog(@"willSelectRowAtIndexPath hahaha");
	} else {

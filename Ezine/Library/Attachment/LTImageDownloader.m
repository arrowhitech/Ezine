...
- (UIImage*)_resizeImage:(UIImage*)image bounds:(CGSize)bounds options:(NSDictionary*)options
{
	...
	CGSize targetSize;
	if (bounds.width > 0) {
		targetSize = sizeThatFitsKeepingAspectRatio2(image.size, CGSizeMake(bounds.width, bounds.width));
	} else {
		targetSize = sizeThatFitsKeepingAspectRatio2(image.size, CGSizeMake(bounds.height, bounds.height));
	}

...
- (id)initWithLine:(CTLineRef)line
{
	...
	//NSLog(@"description===%@",[NSString stringWithFormat:@"<%@ origin=%@ frame=%@ range=%@", [self class], NSStringFromCGPoint(_baselineOrigin), NSStringFromCGRect(self.frame), NSStringFromRange([self stringRange])]);
...
- (CGRect)frame
{
	...
	//NSLog(@"frame text==== %f,%f,%f,%f",_baselineOrigin.x,_baselineOrigin.y,width,_ascent+descent);
